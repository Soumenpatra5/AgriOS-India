...
const FILTER_TABS = [
  ...
  { id: "kid",      label: { en: "Kid",      hi: "बच्चा",    bn: "বাচ্চা"   } },
  { id: "grower",   label: { en: "Grower",   hi: "बढ़ता",    bn: "বেড়ে ওঠা" } },
  { id: "breeding", label: { en: "Breeding", hi: "प्रजनन",   bn: "প্রজনন"   } },
];

const BREEDS = [
  { value: "Beetal",       label: { en: "Beetal",       hi: "बीटल",        bn: "বিটাল"       } },
  ...
  { value: "Osmanabadi",   label: { en: "Osmanabadi",   hi: "उस्मानाबादी",  bn: "উসমানাবাদি"  } },
  { value: "Sangamneri",   label: { en: "Sangamneri",   hi: "संगमनेरी",     bn: "সঙ্গমনেরি"   } },
...
export default function GoatDashboard() {
  ...
  const [aform, setAform] = useState({
    name: "", species: "goat", sex: "unknown", breed: "", tagId: "", dob: "",
    acquisitionDate: today(), acquisitionSource: "", currentStatus: "kid", notes: "",
  });
  ...
  const load = useCallback(async () => {
    try {
      ...
      const [m, list] = await Promise.all([
        goatApi.herdMetrics(active.id),
        goatApi.listAnimals(active.id, { includeTerminal: false }),
      ]);
      setMetrics(m);
      setAnimals(list || []);
      setState("ready");
      if (farmSpaceService.can(active, "farm.goat.finance")) {
        setFinState("loading");
        goatApi.financeSummary(active.id)
      ...
      }
      {
        ...
        goatApi.listMilk(active.id, { fromDate, toDate, limit: 300 })
  ...
  const addAnimal = async () => {
    ...
    setAbusy(true);
    try {
      const animal = await goatApi.createAnimal(space.id, {
      ...
      setAddOpen(false);
      setAform({ name: "", species: "goat", sex: "unknown", breed: "", tagId: "", dob: "",
        acquisitionDate: today(), acquisitionSource: "", currentStatus: "kid", notes: "" });
      toast(tc({ en: "Animal added", hi: "पशु जोड़ा गया", bn: "প্রাণী যোগ হয়েছে" }), "success");
      goatApi.herdMetrics(space.id).then(setMetrics).catch(() => {});
    } catch (err) {
  ...
  const bar = (
    <AppBar
      title={tc({ en: "Goat & Sheep Herd", hi: "बकरी व भेड़ पशु", bn: "ছাগল ও ভেড়া পাল" })}
      onBack={pop}
      action={canManage && (
        <button
  ...
  return (
    <>
      ...
      {/* Kidding due alert */}
      {metrics?.kidding_expected_count > 0 && (
        <div style={{ margin: "8px 16px 0", background: T.blueSoft, borderRadius: T.rMd,
          ...
          <span style={{ fontSize: 12.5, color: T.blue, fontWeight: 600, fontFamily: T.body }}>
            {tc({
              en: `${metrics.kidding_expected_count} animal${metrics.kidding_expected_count > 1 ? "s" : ""} confirmed pregnant this month`,
              hi: `इस माह ${metrics.kidding_expected_count} पशु गर्भवती`,
              bn: `এ মাসে ${metrics.kidding_expected_count}টি প্রাণী গর্ভবতী নিশ্চিত`,
            })}
          </span>
        </div>
      )}
      ...
      {milkHistory !== null && (
        <MilkProductionCard
          ...
          canRecord={canRecord}
          tc={tc}
          onRecord={() => push({ kind: "goatMilkEntry", props: { spaceId: space.id } })}
        />
      )}
      ...
      <div style={{ padding: "6px 16px 32px", display: "flex", flexDirection: "column", gap: 8 }}>
        {filtered.length === 0 ? (
          <EmptyState
            ...
            body={canManage
              ? tc({ en: "Tap + Add to register your first goat or sheep.", hi: "पहली बकरी या भेड़ जोड़ने के लिए + जोड़ें दबाएँ।", bn: "প্রথম ছাগল বা ভেড়া নিবন্ধন করতে + যোগ চাপুন।" })
              : tc({ en: "The herd is empty.", hi: "झुंड खाली है।", bn: "পাল খালি।" })}
          />
        ) : (
          filtered.map((animal) => (
            <AnimalCard key={animal.id} animal={animal} tc={tc} onPress={() =>
              push({ kind: "goatAnimalDetail", props: { animalId: animal.id, spaceId: space.id } })
            } />
          ))
        )}
      </div>
      ...
      <BottomSheet
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <Input
            ...
            placeholder={tc({ en: "e.g. Kali", hi: "उदा. काली", bn: "যেমন কালি" })}
            value={aform.name}
          ...
          <Input
            ...
            placeholder={tc({ en: "e.g. 007", hi: "उदा. 007", bn: "যেমন 007" })}
            value={aform.tagId}
          ...
          <Dropdown
            ...
            options={[
              { value: "kid",      label: tc({ en: "Kid",      hi: "बच्चा",   bn: "বাচ্চা"   }) },
              ...
              { value: "breeding", label: tc({ en: "Breeding", hi: "प्रजनन",   bn: "প্রজনন"   }) },
            ]} />
          <Input
...
function MilkProductionCard({ milkHistory, canRecord, tc, onRecord }) {
  ...
  for (const r of milkHistory) {
    ...
    byDate[key] = (byDate[key] || 0) + (Number(r.total_yield_kg) || 0);
  }
...
function AnimalCard({ animal, tc, onPress }) {
  ...
  const speciesLabel = animal.species === "sheep"
    ...
    <Card pad={0}>
      <button
        ...
        <div style={{ width: 42, height: 42, borderRadius: 12, flexShrink: 0,
          ...
          <Icon name="Beef" size={20} color={cfg.fg} />
        </div>
