...
/* ── Pig finance constants ────────────────────────────────────────────── */

const SALE_TYPE_OPTIONS = [
  { value: "live_animal", label: { en: "Live animal",  hi: "जीवित सूअर",  bn: "জীবন্ত শূকর"  } },
  { value: "pork",        label: { en: "Pork",         hi: "पोर्क मांस",  bn: "শূকরের মাংস"  } },
  { value: "piglet",      label: { en: "Piglet sale",  hi: "बच्चा सूअर",  bn: "শূকরছানা"      } },
...
const SALE_TYPE_COLOR = {
  ...
  pork:        { fg: "#8b2828", bg: "#fde8e8" },
  piglet:      { fg: T.primary, bg: T.primarySoft },
...
export default function PigFinance() {
  ...
  const load = useCallback(async (ymOverride) => {
    try {
      ...
      const [summ, saleList, costList] = await Promise.all([
        pigApi.financeSummary(active.id, { fromDate, toDate }),
        pigApi.listSales(active.id, { fromDate, toDate }),
        pigApi.listCosts(active.id, { fromDate, toDate }),
      ]);
      setSummary(summ);
      setSales(saleList || []);
      setCosts(costList || []);
  ...
  const addSale = async () => {
    ...
    setSbusy(true);
    try {
      const rec = await pigApi.addSale(space.id, {
  ...
  const deleteSale = async () => {
    ...
    try {
      await pigApi.deleteSale(space.id, { saleId: deleteSaleId });
  ...
  const addCost = async () => {
    ...
    setCbusy(true);
    try {
      const rec = await pigApi.addCost(space.id, {
  ...
  const deleteCost = async () => {
    ...
    try {
      await pigApi.deleteCost(space.id, { costId: deleteCostId });
  ...
  const share = async () => {
    ...
    const text = [
      `🐷 Pig Finance Report — ${MONTH_NAMES[ym.month]} ${ym.year}`,
  ...
  const bar = (
    <AppBar
      title={tc({ en: "Pig Finance", hi: "सूअर वित्त", bn: "শূকর অর্থ" })}
      onBack={pop}
      action={(
        ...
        <div style={{ padding: "6px 16px 32px", display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {sales.length === 0 ? (
            <EmptyState icon="TrendingUp"
              ...
              body={tc({ en: "Log a pig or pork sale to get started.", hi: "शुरू करने के लिए एक बिक्री दर्ज करें।", bn: "শুরু করতে একটি বিক্রয় লিখুন।" })} />
          ) : (
            sales.map((s) => (
              <Card key={s.id} pad={0}>
      ...
      <BottomSheet open={saleOpen} onClose={() => setSaleOpen(false)}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <Input label={tc({ en: "Unit (optional)", hi: "इकाई (वैकल्पिक)", bn: "একক (ঐচ্ছিক)" })}
            placeholder={tc({ en: "e.g. animals / kg", hi: "उदा. पशु / किलो", bn: "যেমন প্রাণী / কেজি" })}
      ...
      <BottomSheet open={costOpen} onClose={() => setCostOpen(false)}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <Input label={tc({ en: "Description", hi: "विवरण", bn: "বিবরণ" })}
            placeholder={tc({ en: "e.g. Grower pellets — 50 kg bag", hi: "उदा. ग्रोअर पेलेट्स — 50 किलो बैग", bn: "যেমন গ্রোয়ার পেলেট — ৫০ কেজি ব্যাগ" })}
